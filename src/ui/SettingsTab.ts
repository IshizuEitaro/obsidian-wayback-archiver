...
class WaybackArchiverSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		const profileButtonContainer = containerEl.createDiv({
			cls: "wa-profileButtonContainer",
		});
		...
		new Setting(containerEl)
			...
			.addTextArea((text) =>
				text
					...
					.onChange(async (value) => {
						activeSettings.ignorePatterns = value
							.split("\n")
							.map((p) => p.trim())
							.filter((p) => p.length > 0);
						await this.plugin.saveSettings();
		...
		containerEl.createEl("p", {
			text: "Define patterns to ONLY archive links within notes matching these criteria. If multiple filter types are used, the note/link must match ALL active filter types.",
		});
		...
		new Setting(containerEl)
			...
			.addTextArea((text) =>
				text
					...
					.onChange(async (value) => {
						activeSettings.pathPatterns = value
							.split("\n")
							.map((p) => p.trim())
							.filter((p) => p.length > 0);
						await this.plugin.saveSettings();
		...
		new Setting(containerEl)
			...
			.addTextArea((text) =>
				text
					...
					.onChange(async (value) => {
						activeSettings.urlPatterns = value
							.split("\n")
							.map((p) => p.trim())
							.filter((p) => p.length > 0);
						await this.plugin.saveSettings();
		...
		new Setting(containerEl)
			...
			.addTextArea((text) =>
				text
					...
					.onChange(async (value) => {
						activeSettings.wordPatterns = value
							.split("\n")
							.map((p) => p.trim())
							.filter((p) => p.length > 0);
						await this.plugin.saveSettings();
		...
		containerEl.createEl("p", {
			text: "Apply find/replace rules to URLs before archiving. Useful for removing tracking parameters or normalizing links.",
		});
		...
		new Setting(containerEl).addButton((button) =>
			button.setButtonText("Add substitution rule").onClick(async () => {
				activeSettings.substitutionRules.push({
					find: "",
					replace: "",
					regex: false,
				});
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Only archive if the URL has not been archived within this many days (0 = always archive if not present). Uses SPN API `if_not_archived_within`.",
			)
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						...
						activeSettings.archiveFreshnessDays =
		...
		spnDesc.appendText(
			"These options correspond to parameters available in the Archive.org SPN API v2. ",
		);
		spnDesc.createEl("a", {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(activeSettings.captureScreenshot).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(activeSettings.captureAll).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text
					...
					.onChange(async (value) => {
						const numValue = parseInt(value, 10);
						activeSettings.jsBehaviorTimeout =
							isNaN(numValue) || numValue < 0 ? 0 : numValue;
						await this.plugin.saveSettings();
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(activeSettings.forceGet).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(activeSettings.captureOutlinks).onChange(async (value) => {
	...
	private renderSubstitutionRules(containerEl: HTMLElement): void {
		...
		if (!activeSettings.substitutionRules || activeSettings.substitutionRules.length === 0) {
			containerEl.createEl("p", {
				text: "No substitution rules defined.",
			});
			return;
		}

		activeSettings.substitutionRules.forEach((rule, index) => {
			const ruleEl = containerEl.createDiv({ cls: "wa-ruleEl" });

			const findInput = ruleEl.createEl("input", {
				type: "text",
			...
			const regexToggleLabel = ruleEl.createEl("label", {
				text: "Regex?",
				cls: "wa-regexToggleLabel",
			});
			const regexToggle = regexToggleLabel.createEl("input", {
				type: "checkbox",
			});
	...
	private async handleCreateProfileClick(): Promise<void> {
		const modal = new ProfileNameModal(this.app, async (newName: string | null) => {
			if (newName && !this.plugin.data.profiles[newName]) {
				this.plugin.data.profiles[newName] = {
					...DEFAULT_SETTINGS,
				};
	...
	private async handleRenameProfileClick(): Promise<void> {
		...
		const modal = new ProfileNameModal(
			this.app,
